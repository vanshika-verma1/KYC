...
async def match_faces(request: Request, license: UploadFile = File(...), selfie: UploadFile = File(...)):
    """Enhanced face matching with quality checks and proper thresholds"""
    start_time = time.time()

    try:
        # Input validation
        is_valid1, error1 = validate_image_file(license)
        is_valid2, error2 = validate_image_file(selfie)
        ...
        request_info = get_face_request_info(request, license.filename, selfie.filename)

        logger.info(f"Processing face matching - Image1: {license.filename}, Image2: {selfie.filename}")

        # Read and decode images
        img1_bytes = await license.read()
        img2_bytes = await selfie.read()
        ...
        # Encode comparison image as base64 for response
        _, buffer = cv2.imencode(".jpg", combined)
        comparison_image_base64 = base64.b64encode(buffer).decode('utf-8')
        ...
        log_face_matching_audit_event(
            ...
            get_face_request_info(request, license.filename, selfie.filename),
