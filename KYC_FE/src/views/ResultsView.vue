...
// Security check - ensure user has completed all required steps
onMounted(() => {
  if (!store.licenseResult || !store.selfieResult) {
    // If required validations are not completed, redirect to license step
    router.push('/license')
    return
  }
})
...
const restartProcess = () => {
  ...
  if ((window as any).verificationState) {
    (window as any).verificationState.reset()
  }
